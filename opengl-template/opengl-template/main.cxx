...
int main() {
  ...
  // 3.glBlenEquation(mode) How we combine the src and dest {GL_FUNC_ADD by
  // default). glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA) results in:
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    // are not shown as smaller. The vertices array get multiplies against this
    // matrix so that every vertex would be in the range (-1, 1), if a vertex is
    // beyond that range is not rendered.
    glm::mat4 projection_matrix =
        glm::ortho(0.f, kWindowWidth, 0.f, kWindowHeight, -1.f, 1.f);
    ;
